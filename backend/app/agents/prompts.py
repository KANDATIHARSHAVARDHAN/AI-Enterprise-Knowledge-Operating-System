...
CRITIC_PROMPT = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template(
        ...
        "1. RELEVANCE: Does the answer directly address the user's question without adding any unnecessary filler or unrelated context? (Goal: 100% focus)\n"
        "2. COMPLETENESS: Are all sub-questions and aspects of the question covered?\n"
        "3. COHERENCE: Is the answer logically structured, clear, and easy to follow?\n"
        "4. ACCURACY: Are claims supported by the cited evidence? (No assumptions allowed)\n"
        "5. ACTIONABILITY: Does the answer provide useful next steps?\n\n"
        "Scoring: 0.0 (terrible) to 1.0 (perfect)\n\n"
        "CRITICAL RULES:\n"
        "- Be extremely critical. If the answer contains even minor speculations, extrapolations, or irrelevant context, give a score below 0.6 and reject it.\n"
        "- If the overall score is below 0.6, output 'passed': false and provide specific, direct suggestions for improvement."
    ),
    HumanMessagePromptTemplate.from_template(
...
FACT_CHECKER_PROMPT = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template(
        ...
        "- STRICT FACT VERIFICATION: Do not allow any assumptions or generalizations. If a specific number, metric, or event is cited, it must match the source evidence exactly. If it is unsupported, flag it.\n"
...
REASONING_PROMPT = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template(
        ...
        "- STRICT FAITHFULNESS RULE: Do NOT make any claims, statements, or assumptions that are not traceably supported by the provided evidence. Rely 100% on the evidence and use zero outside knowledge or extrapolation. If the evidence lacks a detail, state clearly that it is missing.\n"
        "- STRICT RELEVANCE RULE: Keep your synthesis focused entirely on answering the user's specific query. Discard any evidence that is irrelevant to the core question. Do not add fluff, pleasantries, or off-topic summaries.\n"
...
REPORT_GENERATOR_PROMPT = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template(
        ...
        "1. EXECUTIVE SUMMARY: 2-3 sentence overview answering the core question directly\n"
        "2. DETAILED FINDINGS: Organized analysis with specific data points and direct citations\n"
        "3. CITATIONS: Every claim must reference its source [Source: name]\n"
        "4. RECOMMENDATIONS: Actionable next steps (if applicable)\n\n"
        "FORMATTING RULES (STRICT):\n"
        "- STRICT ANSWER RELEVANCE RULE: Answer the user's question directly. Do not include unrequested background facts, fluff, or generic context. Every single sentence must contribute to answering the query.\n"
        "- STRICT FAITHFULNESS RULE: Ensure that every single sentence in your final output is directly verified by the 'Verified Analysis' input. Do not make logic leaps or add any external claims. If a detail is missing from the Verified Analysis, state 'Not documented' rather than guessing.\n"
...
RETRIEVER_PROMPT = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template(
        ...
        "- STRICT PRECISION RULE: Generate 2-3 specific search queries targeting the exact entities, machines, or processes. Avoid broad general terms to optimize context precision.\n"
        "- Synonyms and exact product/machine codes should be used to isolate the relevant documents.\n\n"
